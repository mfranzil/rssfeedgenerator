...
rssfile = config.outputpath + "telsy.xml"
list_of_articles = []
...
def make_feed():
    ...
    title.text = "Telsy SpA"

    link = ET.SubElement(channel, "link")
    link.text = "http://rss.draghetti.it"

    description = ET.SubElement(channel, "description")
    description.text = "Feed RSS non ufficiale di tutti gli articoli scritti da Telsy SpA"
...
def scrap_homepage(url):
    ...
    for div in soupdesktop.find_all("div", attrs={"class": "single--news-preview single--news-preview-full-width"}):
        link = div.find("a", href=True)
        list_of_articles.append(link["href"])


def main():
    url = "https://www.telsy.com/blog/"

    # Acquisisco tutti gli URL pubblicati sul sito di telsy
    scrap_homepage(url)
